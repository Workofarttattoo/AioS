...
# Add the current directory to path FIRST to prioritize local aios module
...
from autonomous_discovery import (  # type: ignore  # noqa: E402
    AgentAutonomy,
    create_agi_action,
)
...
def parse_cli(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "command",
        choices=["boot", "shutdown", "status", "list", "exec", "prompt", "sequence", "metadata", "virtualization", "virtualization-domains", "wizard", "autonomy"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Autonomy level for 'autonomy' command (5, 6, or 7).",
    )
    parser.add_argument(
        "--mission",
        type=str,
        default="general research",
        help="Mission string supplied to autonomy demo (Levels 5–7).",
    )
    parser.add_argument(
        "--cycles",
        type=int,
        default=1,
        help="Number of cycles to run for autonomy demo (Levels 5–7).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "autonomy":
        emit_logo()
        # Allow env override for autonomy level
        level_from_env = env_overrides.get("AGENTA_DISCOVERY_AUTONOMY_LEVEL")
        level_int = int(level_from_env) if level_from_env and level_from_env.isdigit() else args.level
        level_map = {
            5: AgentAutonomy.LEVEL_5_ALIGNED,
            6: AgentAutonomy.LEVEL_6_SELF_AWARE,
            7: AgentAutonomy.LEVEL_7_CONSCIOUS,
        }
        if level_int not in level_map:
            LOG.error("[error] Invalid autonomy level %s. Use 5, 6, or 7.", level_int)
            return 2
        try:
            action = create_agi_action(
                mission=args.mission,
                level=level_map[level_int],
                cycles=max(1, int(args.cycles)),
            )
            result = asyncio.run(action())
            LOG.info("[info] autonomy result:\n%s", json.dumps(result, indent=2))
            return 0
        except Exception as exc:
            LOG.error("[error] Autonomy execution failed: %s", exc)
            return 1
