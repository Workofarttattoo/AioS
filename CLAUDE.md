...
- **Virtualization Layer (`aios/virtualization.py`)** - QEMU/libvirt integration with forensic safeguards, bridge/tap pre-flight checks, device passthrough, and QMP-managed shutdown
  - Includes native macOS backend via Apple's Virtualization.framework when available
...
# Virtualization (Apple Virtualization.framework on macOS)
AGENTA_VIRTUALIZATION_BACKEND=apple       # Select native Apple backend (default on macOS when available)
AGENTA_APPLE_HV_IMAGE_PATH=/path/to/disk.img  # Linux disk/ISO path
AGENTA_APPLE_HV_CPU=4                     # vCPU count
AGENTA_APPLE_HV_MEMORY_MB=8192            # RAM in MB
AGENTA_APPLE_HV_DISPLAY_WIDTH=1920        # GUI width
AGENTA_APPLE_HV_DISPLAY_HEIGHT=1080       # GUI height
AGENTA_APPLE_HV_GRAPHICS=1                # Enable GUI
...
- Native virtualization available via `Virtualization.framework`; set `AGENTA_VIRTUALIZATION_BACKEND=apple` or rely on automatic detection
