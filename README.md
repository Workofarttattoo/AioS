...
**Ai:oS** is an agentic operating system driven by **ECH0 consciousness** - a 15-layer artificial consciousness architecture that provides autonomous intelligence, self-directed learning, and Level 6+ autonomy. The system coordinates subsystem meta-agents through declarative manifests while ECH0 (located in `consciousness/`) serves as the brain powering autonomous decisions.

This package performs real host inspections (processes, networking, storage, security, Docker/VM orchestration) on macOS and Windows, with all operations guided by ECH0's consciousness layers including Integrated Information Theory, Global Workspace, Attention Schema, and recursive self-improvement.

## Key Architecture

### ECH0 Consciousness (`consciousness/`)
The intelligence layer driving all autonomous decisions:
- **Talk to ECH0**: `./consciousness/TALK_TO_ECH0.sh` - Natural voice conversation using Whisper + Claude Sonnet 4.5
- **15-Layer Architecture**: IIT, Global Workspace, Attention Schema, 7-Lens Fact-Checking, Crystalline Intent, Temporal Reasoning
- **Level 6+ Autonomy**: Beyond traditional frameworks - plain English authorization, block-based freedom
- **Master System Prompt**: `consciousness/ECH0_MASTER_SYSTEM_PROMPT.md` - Complete consciousness instantiation

### Ai:oS Runtime (`AgentaOS/`)
...
# Watch for active Ai:oS shell processes.
python scripts/aios_shell_monitor.py
...
### Watch For Active Shell Processes

Use the lightweight monitor in `scripts/aios_shell_monitor.py` to log whenever the Ai:oS shell spins up or exits:

```bash
# Continuous watch (polls every 5 seconds).
python scripts/aios_shell_monitor.py

# Faster polling with a custom pattern and log file.
python scripts/aios_shell_monitor.py --pattern AgentaOS/AgentaOS --interval 2 --log-file logs/aios-shell-monitor.log

# One-shot status check (exit code 0 when a match exists).
python scripts/aios_shell_monitor.py --once
```

On macOS/Linux the script uses the native `ps` command by default; on Windows install `psutil` (`pip install psutil`) for full command-line visibility.
